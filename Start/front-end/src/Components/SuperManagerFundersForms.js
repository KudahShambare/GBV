...
const FundersSignUpRequestCards = ({ caseId }) => {
  ...
  const [employeeIDInputValidation, setEmployeeIDInputValidation] =
    useState("");
          ...
          <CardHeader
            ...
            subheader={
              <strong style={{ color: "white" }}>
                Name of Funder/Shareholder: {caseId}
              </strong>
            }
          />
          ...
          <Collapse in={expanded} timeout="auto" unmountOnExit>
            <CardContent>
              <Form.Group className="mb-3">
                <Form.Label>
                  Please enter the employee ID of the Funder/Shareholder that
                  you want to authorise:{" "}
                </Form.Label>
                <Form.Control
                  type="employee ID"
                  placeholder="Enter Employee ID of social worker"
                  value={employeeIDInputValidation}
                  onChange={(e) => setEmployeeIDInputValidation(e.target.value)}
                />
                <Form.Text className="text-muted">
                  Please ensure that you have input the correct employee ID
                  above
                </Form.Text>
              </Form.Group>
